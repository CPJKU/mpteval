...
from mpteval.core.config import ONSET_OFFSET_TOLERANCE_NOTEWISE_EVAL
...
def to_interval_array(note_list):
    if note_list.dtype.names:  # structured array with named fields
        assert 'onset_sec' in note_list.dtype.names and 'duration_sec' in note_list.dtype.names, \
            "Structured array must have 'onset_sec' and 'duration_sec' fields"
        return np.column_stack([
            note_list['onset_sec'],
            note_list['onset_sec'] + note_list['duration_sec'],  # duration -> offset
            note_list['pitch'],
            note_list['velocity'],
        ])
    else:  # plain 2D array, assume (onset, offset, pitch, velocity)
        return note_list
...
def ir_metrics_notewise(ref_notelist, pred_notelist, onset_only=False, verbose=False):
    ...
    ref_notelist = to_interval_array(ref_notelist)
    pred_notelist = to_interval_array(pred_notelist)
    ...
    (
        ...
        pitch_tolerance=0,
        offset_ratio=None if onset_only else 0.2, # type: ignore[assignment]
...
def ir_metrics_notewise_with_velocity(ref_notelist, pred_notelist, onset_only=False, verbose=False):
    ...
    (
        ...
        beta=1.0, # balance b/w precision anad recall; 2.0 recall >> precision
        strict=False,
    )
